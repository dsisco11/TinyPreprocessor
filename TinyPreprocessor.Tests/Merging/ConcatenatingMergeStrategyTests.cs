...
/// <summary>
/// Unit tests for <see cref="ConcatenatingMergeStrategy{TContext}"/>.
/// </summary>
public sealed class ConcatenatingMergeStrategyTests
{
    ...
    public void Merge_CustomSeparator_UsesSeparator()
    {
        ...
        var strategy = new ConcatenatingMergeStrategy<object>(options);
        var resources = CreateResolvedResources(
            ("a.txt", "Part A"),
            ("b.txt", "Part B"));
        var context = CreateMergeContext();
    ...
    public void Merge_MultipleDirectives_StripsAll()
    {
        ...
        var resource = new ResolvedResource(
            new Resource("test.txt", content.AsMemory()),
            new IDirective[] { directive1, directive2 });
        var context = CreateMergeContext();
    ...
    public void Merge_NoDirectives_ContentUnchanged()
    {
        ...
        var resource = new ResolvedResource(
            new Resource("clean.txt", content.AsMemory()),
    ...
    public void Merge_DirectiveAtStart_StripsCorrectly()
    {
        ...
        var resource = new ResolvedResource(
            new Resource("header.h", content.AsMemory()),
            new[] { directive });
        var context = CreateMergeContext();
    ...
    public void Merge_DirectiveAtEnd_StripsCorrectly()
    {
        ...
        var resource = new ResolvedResource(
            new Resource("guarded.h", content.AsMemory()),
            new[] { directive });
        var context = CreateMergeContext();
    ...
    {
        Assert.Throws<ArgumentNullException>(() =>
            new ConcatenatingMergeStrategy<object>(null!));
    }
    ...
    private static List<ResolvedResource> CreateResolvedResources(
        params (string Path, string Content)[] items)
    {
        return items
            .Select(item => new ResolvedResource(
                new Resource(item.Path, item.Content.AsMemory()),
                Array.Empty<IDirective>()))
            .ToList();
    }
